	...
	for claim in expected_claims.keys():
		match claim:
			JWTClaims.Public.EXPIRES_AT:
				if not assert_valid_date_claim(
					jwt_decoder.get_expires_at(), expected_claims.get(claim), true
				):
					self.exception = (
						"The Token has expired on %s."
						% Time.get_datetime_string_from_unix_time(jwt_decoder.get_expires_at())
					)
					return false
			JWTClaims.Public.ISSUED_AT:
				if not assert_valid_date_claim(
					jwt_decoder.get_issued_at(), expected_claims.get(claim), false
				):
					self.exception = (
						"The Token can't be used before %s."
						% Time.get_datetime_string_from_unix_time(jwt_decoder.get_expires_at())
					)
					return false
			JWTClaims.Public.NOT_BEFORE:
				if not assert_valid_date_claim(
					jwt_decoder.get_not_before(), expected_claims.get(claim), false
				):
					self.exception = (
						"The Token can't be used before %s."
						% Time.get_datetime_string_from_unix_time(jwt_decoder.get_expires_at())
...
func assert_claim_value(jwt_decoder: JWTDecoder, claim: String) -> bool:
	...
	match typeof(jwt_decoder.get_claims().get(claim)):
		TYPE_STRING:
			valid_value = (
				jwt_decoder.get_claims().get(claim) != ""
				and jwt_decoder.get_claims().get(claim) != "null"
			)
		TYPE_INT:
			valid_value = (jwt_decoder.get_claims().get(claim) >= 0)
	return (
		jwt_decoder.get_claims().get(claim) != null
		and valid_value
		and jwt_decoder.get_claims().get(claim) == self._claims.get(claim)
	)
...
func assert_valid_date_claim(date: int, leeway: int, should_be_future: bool) -> bool:
	if date == null:
		return true
	return (self._clock - leeway) < date if should_be_future else (self._clock + leeway) > date
...
func verify(jwt: String) -> JWTExceptions:
	...
	if not assert_valid_header(self._jwt_decoder):
